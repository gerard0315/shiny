...
prefixHandler <- function(prefix, handler) {
  force(prefix)
  force(handler)
...
pathPrefixHandler <- function(prefix, handler) {
  ...
  function(req) {
     #dump(req)
    message("req$PATH_INFO ", req$PATH_INFO)
...
routeHandler <- function(prefix, token, handler) {
  ...
  function(req) {
    ...
    if (isTRUE(grepl(pathPattern, req$PATH_INFO))) {
      ...
      req$PATH_INFO <- pathInfo
      testStr <- paste(pathInfo, prefix, '/?token=', token, sep = "")

      message('\n', 'path prefix test ', req$PATH_INFO)
